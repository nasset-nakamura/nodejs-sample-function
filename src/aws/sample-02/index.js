const { S3Client, ListBucketsCommand, DeleteBucketCommand } = require('@aws-sdk/client-s3');
const dayjs = require('dayjs');

const FUNCTION_NAME = 'aws/sample-02';
...
const main = async () => {
  try {
    ...
    logs.push({ message: `[${sequence++}] バケット一を取得` });
    ...
    // https://docs.aws.amazon.com/ja_jp/sdk-for-javascript/v3/developer-guide/s3-example-creating-buckets.html#s3-example-deleting-buckets
    if (response.Buckets.some((bucket) => bucket.Name === bucketName)) {
      logs.push({ message: `[${sequence++}] バケットを削除`, bucketName });
      const response = await s3.send(
        new DeleteBucketCommand({
          Bucket: bucketName,
        })
      );
      if (response.$metadata.httpStatusCode !== 204) throw new Error('バケット削除失敗');
    }
  } catch (ex) {
